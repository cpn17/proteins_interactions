...
class Contact:
    """Represent a contact between two residues."""
    def __init__(self, residue1, residue2, minimum_distance):
        """Initialize a residue contact."""
    ...
    def add_interaction(self, interaction_type):
        """Add an interaction type to the contact."""
        if interaction_type not in self.interaction_types:
            self.interaction_types.append(interaction_type)

def classify_contacts(interface_pairs):
    """Classify interactions for interface residue pairs."""
    contacts = []
    salt_bridges = []
    hydrogen_bonds = []
    for residue1, residue2, minimum_distance in interface_pairs:
        contact = Contact(residue1, residue2, minimum_distance)
        if detect_hydrophobic_contact(residue1, residue2) is not None:
            contact.add_interaction("hydrophobic")
        for bridge in find_salt_bridges(residue1, residue2):
            salt_bridges.append((residue1, residue2, bridge))
            contact.add_interaction("salt_bridge")
        for bond in find_hydrogen_bonds(residue1, residue2):
            hydrogen_bonds.append((residue1, residue2, bond))
        if find_aromatic_interactions(residue1, residue2):
            contact.add_interaction("pi_stacking")
        contacts.append(contact)
    hydrogen_bonds = refine_hydrogen_bonds(hydrogen_bonds, salt_bridges)
    for residue1, residue2, bond in hydrogen_bonds:
        for contact in contacts:
            if contact.residue1 is residue1 and contact.residue2 is residue2:
                contact.add_interaction("hydrogen_bond")
    for contact in contacts:
        if not contact.interaction_types:
            contact.add_interaction("contact")
    return contacts

def count_interactions(contacts):
    """Count contacts for each interaction type."""
    counts = {"hydrophobic": 0,
              ...
              "contact": 0}
    for contact in contacts:
        for interaction_type in contact.interaction_types:
            counts[interaction_type] += 1
    return counts

def write_csv(contacts, chain1, chain2, output_file):
    """Write residue contacts to a CSV file."""
    with open(output_file, "w", newline="") as file:
        writer = csv.writer(file)
        writer.writerow(["chain1", "residue1", "chain2", "residue2",
                         "minimum_distance", "interaction_types"])
        for contact in contacts:
            writer.writerow([chain1.chain_identifier,
                             f"{contact.residue1.residue_name}{contact.residue1.residue_sequence_number}",
                             chain2.chain_identifier,
                             f"{contact.residue2.residue_name}{contact.residue2.residue_sequence_number}",
                             round(contact.minimum_distance, 2),
                             ";".join(contact.interaction_types)])
...
def main():
    ...
    contacts = classify_contacts(interface_pairs)
    print("Number of interface residue pairs:", len(contacts))
    counts = count_interactions(contacts)
    print("Hydrophobic contacts:", counts["hydrophobic"])
    print("Hydrogen bonds:", counts["hydrogen_bond"])
    print("Salt bridges:", counts["salt_bridge"])
    print("Pi-stacking interactions:", counts["pi_stacking"])
    print("Other contacts:", counts["contact"])
    for contact in contacts:
        print(chain1.chain_identifier,
            contact.residue1.residue_name,
            contact.residue1.residue_sequence_number,
            "-",
            chain2.chain_identifier,
            contact.residue2.residue_name,
            contact.residue2.residue_sequence_number,
            "minimum distance:", round(contact.minimum_distance, 2),
            "interactions:", ", ".join(contact.interaction_types))
    write_csv(contacts, chain1, chain2, output_file)
